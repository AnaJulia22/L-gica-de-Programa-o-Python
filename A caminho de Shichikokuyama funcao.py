...
while (palavra_completa == False): 
      palavra = list(input())     
      unir_silabas(palavra)
      for i in range(len(silaba_juntas)):
        if silaba_juntas[i] in hospital:
            if silaba_juntas[i] not in todas_silabas and silaba_juntas[i] not in silaba_correta:
             quantidade_silabas+=1 
             silaba_correta.append(silaba_juntas[i])
      silaba_juntas.clear()  
         ...
         quantidade_silabas = 0
            ...
            for j in range(len(silaba_correta)):
                for a in range(len(hospital)):
                    if silaba_correta[j] == hospital[a] and silaba_correta[j] not in todas_silabas:
                        posicao.append(a)
                        todas_silabas.append(silaba_correta[j])
            for ii in range(0,1):
                unir_silabas(palavra)
